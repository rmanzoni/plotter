...
path_mmm    = '../191126_21h_30m_eee_signal_region/datacards'
path_mem_os = '../191126_22h_52m_eem_os_signal_region/datacards'
path_mem_ss = '../191126_22h_44m_eem_ss_signal_region/datacards'

datacard_wildcard = 'datacard*hnl_m_12*.txt'

print 'loading cards...'
all_datacards_mmm = glob('/'.join([path_mmm, datacard_wildcard]))
all_datacards_mmm = [dd for dd in all_datacards_mmm if 'coarse' not in dd and signal_type in dd]
all_datacards_mmm.sort()
print '... mmm loaded'

all_datacards_mem_os = glob('/'.join([path_mem_os, datacard_wildcard]))
all_datacards_mem_os = [dd for dd in all_datacards_mem_os if 'coarse' not in dd and signal_type in dd]
all_datacards_mem_os.sort()
print '... mem os loaded'

all_datacards_mem_ss = glob('/'.join([path_mem_ss, datacard_wildcard]))
all_datacards_mem_ss = [dd for dd in all_datacards_mem_ss if 'coarse' not in dd and signal_type in dd]
all_datacards_mem_ss.sort()
print '... mem ss loaded'

method = 'asymptotic' # 'toys'
variable = 'hnl_m_12'
categories_to_combine_mmm    = OrderedDict(zip(['lxy_lt_0p5', 'lxy_0p5_to_2p0', 'lxy_mt_2p0'], ['mmm_disp1'   , 'mmm_disp2'   , 'mmm_disp3'   ]))
categories_to_combine_mem_os = OrderedDict(zip(['lxy_lt_0p5', 'lxy_0p5_to_2p0', 'lxy_mt_2p0'], ['mem_os_disp1', 'mem_os_disp2', 'mem_os_disp3']))
categories_to_combine_mem_ss = OrderedDict(zip(['lxy_lt_0p5', 'lxy_0p5_to_2p0', 'lxy_mt_2p0'], ['mem_ss_disp1', 'mem_ss_disp2', 'mem_ss_disp3']))
run_blind = True
flavour = r'$|V|^2_{e}$'
...
for idc_mmm in all_datacards_mmm:
    idc = idc_mmm.split('/')[-1]
    idc_mem_os = '/'.join([path_mem_os, idc])
    idc_mem_ss = '/'.join([path_mem_ss, idc])
    
#     print '\n============================================='
#     print 'mmm', idc_mmm
#     print 'mem', idc_mem_os
#     print 'mem', idc_mem_ss
    ...
    if signal_mass!=8: continue # FIXME! temporary
    ...
    if 'mmm' not in digested_datacards[signal_mass][signal_coupling].keys():
        digested_datacards[signal_mass][signal_coupling]['mmm'] = []

    if 'mem_os' not in digested_datacards[signal_mass][signal_coupling].keys():
        digested_datacards[signal_mass][signal_coupling]['mem_os'] = []

    if 'mem_ss' not in digested_datacards[signal_mass][signal_coupling].keys():
        digested_datacards[signal_mass][signal_coupling]['mem_ss'] = []

    if any([v in idc_mmm for v in categories_to_combine_mmm.keys()]):
        digested_datacards[signal_mass][signal_coupling]['mmm'].append(idc_mmm) 

    if any([v in idc_mem_os for v in categories_to_combine_mem_os.keys()]):
        digested_datacards[signal_mass][signal_coupling]['mem_os'].append(idc_mem_os) 

    if any([v in idc_mem_ss for v in categories_to_combine_mem_ss.keys()]):
        digested_datacards[signal_mass][signal_coupling]['mem_ss'].append(idc_mem_ss) 
...
for mass, couplings in digested_datacards.iteritems():
    ...
    for coupling in couplings.keys():
        ...
        datacards_to_combine_mmm    = couplings[coupling]['mmm']
        datacards_to_combine_mem_os = couplings[coupling]['mem_os']
        datacards_to_combine_mem_ss = couplings[coupling]['mem_ss']
        # gonna combine the cards    
        command = 'combineCards.py'
        for cat, idc in product(categories_to_combine_mmm, datacards_to_combine_mmm):
            if cat in idc:
                command += ' %s=%s ' %(categories_to_combine_mmm[cat],idc)

        for cat, idc in product(categories_to_combine_mem_os, datacards_to_combine_mem_os):
            if cat in idc:
                command += ' %s=%s ' %(categories_to_combine_mem_os[cat],idc)

        for cat, idc in product(categories_to_combine_mem_ss, datacards_to_combine_mem_ss):
            if cat in idc:
                command += ' %s=%s ' %(categories_to_combine_mem_ss[cat],idc)
